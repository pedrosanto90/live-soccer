...
describe('deleteTournament action', () => {
  ...
  it('retorna erro se o utilizador não está autenticado', async () => {
    ...
    const { deleteTournament } = await import('@/lib/actions/tournaments')
  ...
  it('rejeita apagar um torneio que não está em rascunho', async () => {
    vi.mocked(createClient).mockResolvedValueOnce(
      makeSupabaseMock({
        user: mockUser,
        maybeSingle: {
          data: { created_by: mockUser.id, status: 'active' },
        },
      }) as never
    )
  ...
  it('rejeita apagar quando o utilizador não é o criador', async () => {
    vi.mocked(createClient).mockResolvedValueOnce(
      makeSupabaseMock({
        user: mockUser,
        maybeSingle: {
          data: { created_by: 'outro-user', status: 'draft' },
        },
      }) as never
    )
    ...
    expect(result).toEqual({
      success: false,
      error: 'Apenas o criador pode apagar o torneio.',
    })
  })

  it('apaga um torneio em rascunho e redireciona para /dashboard', async () => {
    vi.mocked(createClient).mockResolvedValueOnce(
      makeSupabaseMock({
        user: mockUser,
        maybeSingle: { data: { created_by: mockUser.id, status: 'draft' } },
      }) as never
    )
    const { redirect } = await import('next/navigation')
